...
#Alterando os labels da coluna Frequency
...
#---- Data Frame Clientes
#Para retirar o valor do sexo, fizemos os seguintes passos:
...
# Agora vamos fazer o Join do dataset para trazer o distrito e região de cada cliente.
...
#filtrando e fazendo um df de débito
...
card_issued = tibble(card_issued)

card_issued$issued = as.character(card_issued$issued)
card_issued
card_issued$issued = paste('19',card_issued$issued,sep='')

card_issued$issued = ymd_hms(card_issued$issued)
...
df_card_final
df_card_final$issued = paste('19',df_card_final$issued,sep='')
df_card_final$issued = ymd_hms(df_card_final$issued)
...
df_loan$date = paste('19', df_loan$date,sep='')
df_loan$date = as.Date(ymd(df_loan$date))
df_loan = tibble(df_loan)
df_loan
...
df_canceled = df_loan %>% filter(status == 'cancelled') %>% View()
...
lm(data = st_loan1, formula = status ~ payments ) -> modelo
summary(modelo)

lm(data = st_loan1, formula = status ~ duration ) -> modelo
summary(modelo)

lm(data = st_loan1, formula = status ~ region.x ) -> modelo
summary(modelo)
...
#anÃ¡lise da relaÃ§Ã£o entre amount e status do emprÃ©stimo
...
#anÃ¡lise da relaÃ§Ã£o entre PORC. PARCELA e status do emprÃ©stimo
